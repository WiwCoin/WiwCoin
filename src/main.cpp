...
// Copyright (c) 2016-2017 The WiwCoin Developers
...
#if defined(NDEBUG)
# error "WiwCoin cannot be compiled without assertions."
#endif
...
CBigNum bnProofOfStakeLimit(~uint256(0) >> 46);
...
const string strMessageMagic = "WiwCoin Signed Message:\n";
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    LogPrintf("SetBestChain: new best=%s  height=%d  date=%s\n",
      hashBestChain.ToString(), nBestHeight,
      DateTimeStrFormat("%x %H:%M:%S", pindexBest->GetBlockTime()));
...
bool CBlock::AcceptBlock()
{
    ...
    if (nVersion < CURRENT_VERSION)
            return DoS(100, error("AcceptBlock() : reject old block version %d", nVersion));
