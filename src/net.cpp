...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "WiwCoin " + FormatFullVersion();
...
bool OpenNetworkConnection(const CAddress& addrConnect, bool fCountFailure, CSemaphoreGrant *grantOutbound, const char *strDest, bool fOneShot)
{
    //
    ...
    CNode* pnode = ConnectNode(addrConnect, strDest, fCountFailure);
    boost::this_thread::interruption_point();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. WiwCoin is probably already running."), addrBind.ToString());
        else
