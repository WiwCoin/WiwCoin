...
{
    // Convert WiwCoin:// to WiwCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("WiwCoin://"))
    {
        uri.replace(0, 12, "WiwCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "WiwCoin.lnk";
}
...
{
    return GetAutostartDir() / "WiwCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("WiwCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  WiwCoin-qt [" + tr("command-line options") + "]                     " + "\n";
